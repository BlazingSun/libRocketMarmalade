...
void init() {
    ...
    s3eFileList* fileList = s3eFileListDirectory("./font/");
    char filename[256];
    while (s3eFileListNext(fileList, filename, 256) == S3E_RESULT_SUCCESS)
    {
        std::string path = std::string("./font/") + filename;
        unsigned pos = path.find_last_of(".");
        std::string extension = path.substr(pos + 1);
        if (extension.compare("ttf") == 0 || extension.compare("otf") == 0)
        {
